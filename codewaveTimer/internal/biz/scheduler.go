...
func NewSchedulerUseCase(confData *config.Data, timerRepo JobRepo, taskRepo TimerTaskRepo, redisCache RedisCache, trigger *TriggerUseCase) *SchedulerUseCase {
...
func (w *SchedulerUseCase) handleSlice(ctx context.Context, bucketID int) {
	...
	defer func() {
		// 捕获异常，继续执行剩下的任务
		if r := recover(); r != nil {
			log.ErrorLog(ctx, "handleSlice %v run err. Recovered from panic:%v", bucketID, r)
		}
	}()

	log.InfoLog(ctx, "scheduler_%v start: %v", bucketID, time.Now())
	now := time.Now()
	...
	if err := w.pool.Submit(func() {
		...
		log.ErrorLog(ctx, "[handle slice] submit task failed, err: %v", err)
	}
	...
	log.InfoLog(ctx, "scheduler_%v end: %v", bucketID, time.Now())
...
func (w *SchedulerUseCase) asyncHandleSlice(ctx context.Context, t time.Time, bucketID int) {
	...
	err := locker.Lock(ctx)
	if err != nil {
		log.InfoLog(ctx, "asyncHandleSlice 获取分布式锁失败: %v", err.Error())
		// 抢锁失败, 直接跳过执行, 下一轮
		return
	}

	log.InfoLog(ctx, "get scheduler lock success, key: %s", utils.GetTimeBucketLockKey(t, bucketID))
	...
	ack := func() {
		if err := locker.DelayExpire(ctx, int64(w.confData.Scheduler.SuccessExpireSeconds)); err != nil {
			log.ErrorLog(ctx, "expire lock failed, lock key: %s, err: %v", utils.GetTimeBucketLockKey(t, bucketID), err)
		}
	}

	if err := w.trigger.Work(ctx, utils.GetSliceMsgKey(t, bucketID), ack); err != nil {
		log.ErrorLog(ctx, "trigger work failed, SliceMsgKey[%v] err: %v", utils.GetSliceMsgKey(t, bucketID), err)
	}
}
