...
func newCacheShard(id int, onRemove OnRemoveCallback, close chan struct{}) *cacheShard {
...
func (cs *cacheShard) expireCleanup() {
	...
	for {
		...
		for key, ele := range cs.expireItems { // 遍历过期key
			...
			if now.Sub(item.CreatedAt()) >= item.LifeSpan() { // 过期
				// del
				delete(cs.items, key)
				...
				if cs.isVerbose {
					cs.logger.Printf("[shard %d]: expire del key <%s>  createdOn:%v,  lifeSpan:%d ms \n", cs.id, key, item.CreatedAt(), item.LifeSpan().Milliseconds())
				}
			} else {
				d := item.LifeSpan() - now.Sub(item.CreatedAt())
